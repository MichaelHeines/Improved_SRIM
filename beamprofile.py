...
# Define beam parameter class
class beam_parameters:
    def __init__(self, y_offset, z_offset, y_std, z_std, angle, nr_particles, atomic_number, energy, energy_std_fractional):
        self.y_offset = y_offset                                # Y offset of beam spot
        self.z_offset = z_offset                                # Z offset of beam spot
        self.y_std = y_std                                      # standard deviation of the beam in the Y direction
        self.z_std = z_std                                      # standard deviation of the beam in the Z direction
        self.angle = angle                                      # angle of implantation
        self.nr_particles = nr_particles                        # number of particles simulated
        self.atomic_number = atomic_number                      # Atomic number of simulated particles
        self.energy = energy                                    # Requested energy
        self.energy_std_fractional = energy_std_fractional      # Energy deviation (not properly implemented yet)

        
# Generate informative text about dat file
...
# Generate 5 char number strings
...
def generate_dat(beam_parameters, save_path, file_name):
    ...
    # Get a beam direction
    phi = np.zeros(nr_particles)                                    # np.random.uniform(0, 2 * math.pi, nr_particles)
    theta = angle * np.ones(nr_particles)
...
parameters = beam_parameters(0, 0, 0, 0, 0, 10**5, 19, 60 * 10**3, 0)
